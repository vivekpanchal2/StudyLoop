...
function CourseDetails() {
  ...
  const studentsEnrolled =
    response?.courseDetails?.studentsEnrolled?.length > 0
      ? response?.courseDetails?.studentsEnrolled
      : [];
        ...
        <div className="mx-auto box-content px-4 lg:w-[1260px] 2xl:relative ">
          <div className="mx-auto grid min-h-[450px] max-w-maxContentTab justify-items-cente py-8 lg:mx-0 lg:justify-items-start lg:py-0 xl:max-w-[810px]">
            ...
            <div className="flex w-full flex-col gap-4 border-y border-y-richblack-500 py-4 lg:hidden">
              ...
              <button
                className="cursor-pointer rounded-md bg-yellow-50 px-[20px] py-[8px] font-semibold text-richblack-900 hover:bg-white  duration-300"
                onClick={
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {tag &&
                tag.map((item, ind) => (
                  <p
                    key={ind}
                    className="bg-yellow-50  px-[20px] py-1 m-[3px] text-black rounded-full text-center font-medium"
                  >
                    {item}
                  </p>
                ))}
